{% macro dbt_snowflake_validate_get_incremental_strategy(config) %}
  ...
  {% do return(strategy) %}
{% endmacro %}
{% macro dbt_snowflake_get_incremental_sql(strategy, tmp_relation, target_relation, unique_key, dest_columns, timestamp_column) %}
  {% if strategy == 'merge' %}
  ...
  {% elif strategy == 'scd+merge' %}
    ...
    {% do return(get_scd_merge_sql(target_relation, tmp_relation, unique_key, dest_columns, none, timestamp_column))  %}
  {% else %}
...
{% materialization scd_incremental, adapter='snowflake' -%}
 ...
 -- setup
  ...
  {% else %}
    ...
    {% set build_sql = dbt_snowflake_get_incremental_sql(strategy, tmp_relation, target_relation, unique_key, dest_columns, timestamp_column) %}
  {% endif %}
  {%- call statement('main') -%}
    {{ build_sql }}
  {%- endcall -%}
