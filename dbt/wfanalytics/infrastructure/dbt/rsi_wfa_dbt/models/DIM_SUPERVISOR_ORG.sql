{{
    config(
        ...
        unique_key ='SUP_ORG_SK',
        post_hook='{{ update_dim_supervisor_org_disappear() }}'
    )
}}
...
WITH RAWDATA AS (
    ...
    {% if is_incremental() %}
    WHERE RPT_EFFECTIVE_DT > ( SELECT MAX(RPT_EFF_DT) FROM {{this}}  )
    {% endif %}
    QUALIFY ROW_NUMBER() OVER(PARTITION BY ORG_ID ORDER BY RPT_EFFECTIVE_DT desc) = 1
),

REQDATA AS (
    SELECT * 
    FROM {{ source('STAGING','STG_WKD_SUPERVISOR_ORG') }} 
    WHERE ORG_ID IN (SELECT ORG_ID FROM RAWDATA )
),
...
STG_ORG AS (
    ...
    ROW_NUMBER() OVER (PARTITION BY ORG_ID ORDER BY RPT_EFFECTIVE_DT) AS RN,
...
,VERSION AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY COALESCE(ORG_ID,'') ORDER BY RPT_EFF_DT DESC) AS VERSION_NUMBER
    FROM DE_DUP
),
