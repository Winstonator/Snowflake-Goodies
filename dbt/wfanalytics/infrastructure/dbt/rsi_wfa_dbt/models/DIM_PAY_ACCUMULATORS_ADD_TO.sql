...
STG_PAY_ACC AS (
    ...
    PAY_ACCUMULATION_CD,
    'WKD' AS REC_SRC, 
    A.VALUE::STRING AS ADD_TO_PAY_ACCUMULATION
    FROM {{ source('STAGING','STG_WKD_PAY_ACCUMULATORS') }},
    LATERAL FLATTEN(INPUT => SPLIT(ADD_TO_PAY_ACCUMULATION, ';')) A
    QUALIFY ROW_NUMBER() OVER(PARTITION BY PAY_ACCUMULATION_WID, ADD_TO_PAY_ACCUMULATION ORDER BY SF_INSERT_TIMESTAMP DESC) = 1
),
...
SELECT 
    {{ dbt_utils.surrogate_key(['PAY_ACCUMULATION_WID','ADD_TO_PAY_ACCUMULATION'])}} AS ADD_TO_SK,
    PAY_ACCUMULATION_WID || '-' || ADD_TO_PAY_ACCUMULATION AS ADD_TO_ID,
    PAY_ACCUMULATION_WID,
    PAY_ACCUMULATION_CD,
    TRIM(ADD_TO_PAY_ACCUMULATION) AS ADD_TO_PAY_ACCUMULATION,
...
LEFT JOIN DIM_PAY_EARN
    ON TRIM(STG_PAY_ACC.ADD_TO_PAY_ACCUMULATION) = TRIM(DIM_PAY_EARN.CALC_CD)
LEFT JOIN DIM_PAY_DEDUCT
    ON TRIM(STG_PAY_ACC.ADD_TO_PAY_ACCUMULATION) = TRIM(DIM_PAY_DEDUCT.CALC_CD)
LEFT JOIN DIM_PAY_COMP 
    ON TRIM(STG_PAY_ACC.ADD_TO_PAY_ACCUMULATION) = TRIM(DIM_PAY_COMP.PAY_COMPONENT_GROUP_NAME)
LEFT JOIN DIM_PAY_CALCS
    ON TRIM(STG_PAY_ACC.ADD_TO_PAY_ACCUMULATION) = TRIM(DIM_PAY_CALCS.PAYROLL_CALC)
